---
name: orchestration
description: Agent orchestration patterns for Nerdy Ad Engine. Use when starting work on features, planning task decomposition, dispatching agents, coordinating parallel work, managing worktrees, reviewing agent output, or handling agent failures.
---
...
```
Task 1: Create Drizzle schema for ads/briefs    [independent]
Task 2: Create LangGraph pipeline nodes         [blocked by 1]
Task 3: Create Elysia API routes                [blocked by 2]
Task 4: Create Meta Ad Library scraper           [independent]
Task 5: Wire end-to-end pipeline                 [blocked by 1-4]
Task 6: Create Next.js dashboard                 [blocked by 3]
```
...
Run `bun run biome check . && bunx tsc --noEmit && bun test` before finishing.
...
All plans must be saved to `memory-bank/plans/` as markdown files with descriptive names (e.g., `memory-bank/plans/implement-langgraph-pipeline.md`). This ensures plans survive context clears.
